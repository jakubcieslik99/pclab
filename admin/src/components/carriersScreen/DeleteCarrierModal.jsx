...
import { successReset, errorReset, deleteCarrier } from '../../features/carriersSlices/deleteCarrier'
...
const DeleteCarrierModal = props => {
  //variables
  const deleteCarrierAbort = useRef()

  const { loading, success, successMessage, error, errorMessage } = useAppSelector(state => state.deleteCarrier)
  const dispatch = useAppDispatch()

  //handlers
  const submitHandler = e => {
    e.preventDefault()
    const deleteCarrierPromise = dispatch(deleteCarrier({ id: props.deleteElement._id }))
    deleteCarrierAbort.current = deleteCarrierPromise.abort
  }
  ...
  const closeHandler = () => {
    ...
    setTimeout(() => {
      ...
      if (deleteCarrierAbort.current) {
        deleteCarrierAbort.current()
        deleteCarrierAbort.current = undefined
