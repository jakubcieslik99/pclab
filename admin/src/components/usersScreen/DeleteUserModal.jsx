...
import { successReset, errorReset, deleteUser } from '../../features/usersSlices/deleteUser'
...
const DeleteUserModal = props => {
  //variables
  const deleteUserAbort = useRef()

  const { loading, success, successMessage, error, errorMessage } = useAppSelector(state => state.deleteUser)
  const dispatch = useAppDispatch()

  //handlers
  const submitHandler = e => {
    e.preventDefault()
    const deleteUserPromise = dispatch(deleteUser({ id: props.deleteElement._id }))
    deleteUserAbort.current = deleteUserPromise.abort
  }
  ...
  const closeHandler = () => {
    ...
    setTimeout(() => {
      ...
      if (deleteUserAbort.current) {
        deleteUserAbort.current()
        deleteUserAbort.current = undefined
              ...
              <Dialog.Panel className="grid items-center w-full max-w-md">
                ...
                >
                  ...
                  <div className="flex flex-col w-full gap-[10px] my-4 overflow-y-auto text-gray-800 text-center">
                    ...
                    <div>
                      Czy na pewno chcesz całkowicie usunąć wybranego użytkownika{' '}
                      <span className="font-bold">{props.deleteElement?.nick}</span>?
                    </div>
                    <div>
